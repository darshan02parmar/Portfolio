  ...
  const unlockSecret = (key: string) => {
    setUnlockedSecrets((prev) => {
      ...
      toast.custom(
        (t) => (
          <div
            className={`${t.visible ? "scale-100 opacity-100" : "scale-90 opacity-0"} max-w-xs w-full bg-amber-400 border-[3px] border-slate-900 p-4 rounded-2xl shadow-[4px_4px_0px_0px_#1e293b] font-mono text-[11px] font-bold text-slate-950 flex items-center gap-3 transition-all duration-300 pointer-events-auto`}
          >
            <span className="text-xl">🏆</span>
            <div>
              <div>ACHIEVEMENT UNLOCKED!</div>
              <div className="text-[9px] text-slate-800 font-normal">
                Found secret: {key.toUpperCase()} ({next.length}/5)
              </div>
            </div>
          </div>
        ),
      ...
      if (next.length === 5) {
        setTimeout(() => {
          ...
          toast.success(
            "🎉 Incredible! You discovered all 5 Developer Secrets!",
            {
              style: {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (konamiBuffer.join(",") === konamiCode.join(",")) {
        ...
        toast.custom(
          (t) => (
            <div
              className={`${t.visible ? "scale-100 opacity-100" : "scale-90 opacity-0"} max-w-sm w-full bg-slate-900 border-[3px] border-[#eab308] p-4 rounded-2xl shadow-[6px_6px_0px_0px_#eab308] font-mono text-[11px] font-bold text-white flex flex-col gap-1 transition-all duration-300 pointer-events-auto`}
            >
              <div className="text-[#eab308] text-xs">
                🚀 DEVELOPER GOD MODE ACTIVATED
              </div>
              <div className="text-[9px] text-slate-400 font-normal">
                Gravity inverted, confetti engine primed, secrets count
                incremented.
              </div>
            </div>
          ),
